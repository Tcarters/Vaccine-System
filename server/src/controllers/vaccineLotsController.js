...
exports.create = async (req, res) => {
    try {
        ...
        console.log('Error create vaccineLots: ', err);
...
exports.getAll = async (req, res) => {
    try {
       const list = await VaccineLot.find({}).populate('vaccine').sort('-createdAt');
        ...
        console.log('Error getAll vaccineLots: ', err);
        res.status(500).json(err);
    }
}
...
exports.getOne = async (req, res) => {
    try {
       const vaccineLot = await VaccineLot.findById(req.params.id).populate('vaccine');
        ...
        console.log('Error getOne vaccineLots: ', err);
        res.status(500).json(err);
    }
}
...
exports.update = async (req, res) => {
    try {
       const vaccineLot = await VaccineLot.findByIdAndUpdate(
        ...
        console.log('Error Update vaccineLots: ', err);
        res.status(500).json(err);
    }
}
...
exports.delete = async (req, res) => {
    try {
        ...
        res.status(200).json('VaccineLot deleted!');
    } catch(err) {
        console.log('Error Deleting  vaccineLots: ', err);
        res.status(500).json(err);
    }
}